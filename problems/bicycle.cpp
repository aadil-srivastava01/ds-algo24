...
Write a function that returns the maximum possible total speed or the minimum possible total speed of all of the tandem bicycles being ridden based on an input parameter, fastest. If fastest = true, your function should return the maximum possible total speed; otherwise It
should return the minimum total speed. "Total speed" is defined as the sum of the speeds of all the tandem bicycles being ridden. For example, if there are 4 riders (2 red-shirt riders and 2 blue-shirt riders) who have speeds of
    ...
    vector<int> &redShirtSpeeds, vector<int> &blueShirtSpeeds, bool fastest)
{
    ...
    int speed{0};
    int ptr1{0};
    int ptr2 = fastest ? (redShirtSpeeds.size() - 1) : 0;

    if (fastest)
    {
        while (ptr2 > -1)
        {
            speed += max(redShirtSpeeds.at(ptr1), blueShirtSpeeds.at(ptr2));
            ++ptr1;
            --ptr2;
        }
        return speed;
    ...
    {
        while (ptr2 < redShirtSpeeds.size())
        {
            speed += max(redShirtSpeeds.at(ptr1), blueShirtSpeeds.at(ptr2));
            ++ptr1;
            ++ptr2;
